...
conf_list = ['CVPR2017','CVPR2016','ICCV2017','ECCV2018']
for this_conf in conf_list:
    ...
    all_links_url = [s.get('href') for s in links]
    all_links_text = [s.get_text() for s in links]
    ...
    titles_text = [s.get_text() for s in titles]
    titles_url = [s.find('a').get('href') for s in titles]
    ...
    for this_title_url in titles_url:
        ...
        try:
            this_title_url_full = url_root+this_title_url
    ...
    for this_paper_dict in paper_dict_list:
        ...
        if target_state:
            ...
            with open('/'.join([folder_name,filename]),'wb') as f:
